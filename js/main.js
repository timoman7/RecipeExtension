    ...
    if(propType == "Single"){
      ...
      for(let i = 0; i < prop.length; i++){
        ...
        if(prop[i].querySelector){
          console.log()
          if(prop[i].querySelector('.recipe-directions__list--item')){
    ...
    for(let schemaProp in schemaOpts[schemaType]){
      ...
      if(prop){
        if(prop.setAttribute){
          console.log(prop)
          prop.setAttribute('itemprop', Object.getOwnPropertyNames(altNames).includes(schemaProp) ? altNames[schemaProp] : schemaProp);
        }
      }
      _schema[Object.getOwnPropertyNames(altNames).includes(schemaProp) ? altNames[schemaProp] : schemaProp] = parseProp(prop, propType, schemaType, origin);
    }
